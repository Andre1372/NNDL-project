...
\author{Jacopo Pegoraro$^\dag$, Michele Rossi$^\dag$,
...
The network we developed is a General Adversarial Network (GAN) that is modified by including a Contitioner network inside the Generator (CGAN) in order to help it use past previously generated bars to build more audible sequences of notes and chords.
In the generation phase, the network is forced to use previously generated bars to define the new ones.
In addition, we included a Gated Recurrent Unit (GRU) to maintain an hidden state that, combined with the features extracted by the Conditioner, is able to maintain a theme of the piece of music that is being generated, which is itself used at the beginning of the generation process, in order to force the generation of coherent melodies.

The result are short pieces of piano music, composed of melody and chords, that tend to as more realistic.
Even if the music generation is limited to short and piano music, improvements on this architecture could lead to the generation of more complex and realistic compositions\\ 
