...
\begin{abstract}
  ...
  Inspired by Midinet \cite{Yang-2017}, our system is a conditioned Generative Adversarial Network (GAN) is enhanced with a set of residual blocks and a dedicated Conditioner network that extracts features from previous bars to ensure temporal continuity.
  To produce a stable learning process, we adopt a Wasserstein loss with gradient penalty (WGAN-GP), replacing the standard binary cross-entropy. Also, the Critic (as we will call the Discriminator network, due to the new loss function) is enhanched with a Minibatch Standard Deviation layer, which provides information that help in penalizing repetitive outputs.
  The result is an integration of recurrent memory and convolutional layers that leads to improved thematic coherence.
  The integration of these systems is used to produce piano music from an underlying chord progression, genrating coherent music.
