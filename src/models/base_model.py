...
class Generator(nn.Module):
    def __init__(self, input_size):
        ...
        # Input Concat Channels: 256 (from reshape) + 256 (from conditioner) + self.chord_dim (from chords)
        self.transp_conv_1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=512+self.chord_dim, out_channels=256, kernel_size=(1, 2), stride=(1, 2)),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2)
        )
        ...
        self.transp_conv_4 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=512+self.chord_dim, out_channels=1, kernel_size=(128, 1), stride=1),
    ...
    def forward(self, z, condition_matrix, chord_idx, hidden_state=None):
        # Conditioner on the previous bar
        ...
        # Conditioner on the chord
        chord_vector = self.chord_embedding(chord_idx)
        
        # Generator process noise
        linear_output = self.ff_nets(z)
        reshaped_output = self.reshape(linear_output + new_hidden_state)
        
        # Helper function per concatenare gli accordi
        def concat_chords(feature_map, cond_map, chord_vec):
            # feature_map: (B, C1, H, W)
            # cond_map: (B, C2, H, W)
            # chord_vec: (B, self.chord_dim)
            b, _, h, w = feature_map.shape
            # Reshape e Expand: (B, self.chord_dim) -> (B, self.chord_dim, 1, 1) -> (B, self.chord_dim, H, W)
        ...
        merged1 = concat_chords(reshaped_output, condition_step4, chord_vector) # Canali: 256+256+self.chord_dim
        output_step1 = self.transp_conv_1(merged1)                      # Output: (B, 256, 1, 4)    

        # Step 2: Out1 + Cond3 + Chords
        merged2 = concat_chords(output_step1, condition_step3, chord_vector)         # Canali: 256+256+self.chord_dim
        output_step2 = self.transp_conv_2(merged2)                      # Output: (B, 256, 1, 8)

        # Step 3: Out2 + Cond2 + Chords
        merged3 = concat_chords(output_step2, condition_step2, chord_vector)         # Canali: 256+256+self.chord_dim
        output_step3 = self.transp_conv_3(merged3)                      # Output: (B, 256, 1, 16)

        # Step 4: Out3 + Cond1 + Chords
        merged4 = concat_chords(output_step3, condition_step1, chord_vector)         # Canali: 256+256+self.chord_dim
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.ff_layers = nn.Sequential(
            nn.Linear(in_features=64*3*2 + self.chord_dim, out_features=512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.3),
    ...
    def forward(self, x_curr, x_prev, chord_idx):
        ...
        combined = torch.cat((flat_curr, flat_prev, chord_vector), dim=1)
        output = self.ff_layers(combined)

        return output, feat_curr
